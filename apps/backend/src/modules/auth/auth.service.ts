import type { TOAuthUser } from "@/modules/auth/auth.types";
...
import { SignInInput, SignUpInput } from "@/modules/auth/auth.dto";
...
export class AuthService {
	...
	public async singIn(dto: SignInInput): Promise<TUserWithAvatar> {
		...
		const isValidPassword = await this.hashingService.verify(user.hash, dto.password);
	...
	private async handleFailedLogin(userId: string, email: string) {
		...
		const lockedUntil = shouldLock ? new Date(Date.now() + this.LOCK_DURATION) : null;
		...
		if (shouldLock) {
			this.logger.warn(`Account locked due to ${attempts} failed attempts. User ID: ${userId}, Email: ${email}`);
		} else {
			this.logger.warn(
