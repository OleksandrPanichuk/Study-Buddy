...
export class TutorChatsController {
	...
	@Get("")
	findByUserId(@Body() dto: FindAllTutorChatsInput, @CurrentUser("id") userId: string) {
		return this.tutorChatsService.findAll(dto, userId);
	}
	...
	@HttpCode(HttpStatus.NO_CONTENT)
	@Delete("/bulk")
	deleteBulk(@Body() dto: BulkDeleteTutorChatsInput, @CurrentUser("id") userId: string) {
