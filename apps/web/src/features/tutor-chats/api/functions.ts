...
export const getAllTutorChatsFn = createServerFn({ method: "GET" })
	.inputValidator((data: TFindAllTutorChatsInput) =>
		findAllTutorChatsInputSchema.parse(data),
	)
	.handler(async (ctx) => {
...
export const createTutorChatFn = createServerFn({ method: "POST" })
	.inputValidator((data: TCreateTutorChatInput) =>
		createTutorChatInputSchema.parse(data),
	)
	.handler(async (ctx) => {
