...
export class MailerService implements OnModuleInit {
	...
	public async sendWelcomeEmail(email: string, userName: string, dashboardLink: string): Promise<void> {
	...
	public async sendEmail(options: IEmailOptions): Promise<void> {
		...
		const replyToAddresses = replyTo ? (Array.isArray(replyTo) ? replyTo.join(", ") : replyTo) : undefined;
	...
	private async verifyTransporter(): Promise<void> {
		try {
			...
			if (result) {
				this.logger.log("SMTP connection verified - transporter is ready to send emails");
			}
		} catch (error) {
			this.logger.error("Failed to verify SMTP connection", error instanceof Error ? error.stack : error);
			throw new Error("SMTP transporter verification failed. Check your mail configuration.");
		}
	}
	...
	private async sendWithRetry(
		...
		try {
			const info: SMTPTransport.SentMessageInfo = await this.transporter.sendMail(mailOptions);
			...
			if (attempt < this.MAX_RETRIES) {
				...
				return this.sendWithRetry(mailOptions, recipients, template, attempt + 1);
			}
