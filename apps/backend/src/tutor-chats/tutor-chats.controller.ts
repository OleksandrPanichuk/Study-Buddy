...
export class TutorChatsController {
	...
	@Get("/")
	findByUserId(@Query() query: FindAllTutorChatsQuery, @CurrentUser("id") userId: string) {
	...
	delete(@Param() params: DeleteTutorChatParams, @CurrentUser("id") userId: string) {
	...
	@HttpCode(HttpStatus.NO_CONTENT)
	@Delete("/bulk")
	deleteBulk(@Query() query: BulkDeleteTutorChatQuery, @CurrentUser("id") userId: string) {
		return this.tutorChatsService.bulkDelete(query.ids, userId);
	}
}
