...
export async function tryCatch<T>(promise: T | Promise<T>): Promise<TryCatchResult<T>> {
	try {
...
export async function ensureCurrentUser(queryClient: QueryClient) {
	const user = queryClient.getQueryData<TUserWithAvatar | null>(PROFILE_QUERY_KEYS.currentUser());
...
export const isInfiniteQuery = <T>(v: unknown): v is InfiniteData<T> => !!v && typeof v === "object" && "pages" in v;
