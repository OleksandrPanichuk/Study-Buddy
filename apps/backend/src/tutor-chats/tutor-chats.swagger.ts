...
import {BulkDeleteTutorChatQuery, CreateTutorChatInput, FindAllTutorChatsQuery} from "./tutor-chats.dto";

export const ApiFindAllTutorChats = () => {
	return applyDecorators(
		ApiOperation({
			summary: "Get all tutor chats for the authenticated user",
			description: "Retrieves a list of tutor chats associated with the authenticated user."
		}),
		ApiQuery({
...
export const ApiDeleteTutorChat = () => {
	return applyDecorators(
		...
		ApiNotFoundResponse({
			description: "The tutor chat could not be found or tutor chat does not belong to the requesting user."
		})
	);
};
